...
export async function register(email, password) {
    const { token, account } = await openhackApi.Accounts.register({
        email,
        password,
    });
    setAuthToken(token);
...
export async function login(email, password) {
    const { token, account } = await openhackApi.Accounts.login({
        email,
        password,
    });
    setAuthToken(token);
