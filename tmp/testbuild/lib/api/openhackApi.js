...
export function createApiHelpers(apiClient = api) {
    const Accounts = {
        ...
        check: (email) => request(() => apiClient.post('/accounts/check', { email })),
        // register issues a token and profile for new accounts.
        register: (payload) => request(() => apiClient.post('/accounts/register', payload)),
        // login authenticates via credentials.
        login: (payload) => request(() => apiClient.post('/accounts/login', payload)),
        // whoami resolves the current account from the auth token.
        whoami: () => request(() => apiClient.get('/accounts/whoami')),
        // editName updates the display name and returns a refreshed token payload.
        editName: (name) => request(() => apiClient.patch('/accounts', { name })),
    ...
    const Teams = {
        ...
        rename: (payload) => request(() => apiClient.patch('/teams', payload)),
        ...
        join: (id) => request(() => apiClient.patch('/teams/join', undefined, { params: { id } })),
        // leave removes the caller and returns the remaining roster.
        leave: () => request(() => apiClient.patch('/teams/leave')),
        // kick expels the given account ID and returns the updated roster.
        kick: (id) => request(() => apiClient.patch('/teams/kick', undefined, { params: { id } })),
    };
    const Submissions = {
