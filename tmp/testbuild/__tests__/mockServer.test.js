...
import { createApiHelpers, openhackApi, isApiError, } from '../lib/api/openhackApi.js';
import { accountRune, logout, whoami } from '../runes/accountRune.js';
import { teamRune, teamMembersRune, createTeam, join, leave, kick, deleteTeam, renameTeam, loadMembers, } from '../runes/teamRune.js';
...
function createAxiosError(status, data) {
...
function createMockAxios(handlers) {
    ...
    const instance = {
        ...
        request: (config) => call((config?.method ?? 'get'), config?.url ?? '', config?.data, config),
...
async function runHelperTests() {
    const helpers = createApiHelpers(createMockAxios({
...
async function runRuneTests() {
    ...
    openhackApi.Teams.create = async (name) => ({ token: 'tkn', account: sampleAccount });
